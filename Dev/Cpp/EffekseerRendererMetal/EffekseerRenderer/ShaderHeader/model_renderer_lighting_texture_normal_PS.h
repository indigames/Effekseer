...
float4 _main(PS_Input Input, thread texture2d<float> g_uvDistortionTexture, thread sampler g_uvDistortionSampler, constant PS_ConstanBuffer& v_133, thread texture2d<float> g_colorTexture, thread sampler g_colorSampler, thread texture2d<float> g_normalTexture, thread sampler g_normalSampler, thread texture2d<float> g_alphaTexture, thread sampler g_alphaSampler, thread texture2d<float> g_blendUVDistortionTexture, thread sampler g_blendUVDistortionSampler, thread texture2d<float> g_blendTexture, thread sampler g_blendSampler, thread texture2d<float> g_blendAlphaTexture, thread sampler g_blendAlphaSampler)
{
    ...
    Output = param_2;
    float diffuse = fast::max(dot(v_133.fLightDirection.xyz, localNormal), 0.0);
    float3 _304 = Output.xyz * ((v_133.fLightColor.xyz * diffuse) + v_133.fLightAmbient.xyz);
    Output = float4(_304.x, _304.y, _304.z, Output.w);
    if (v_133.fFalloffParam.Param.x == 1.0)
    {
        ...
        {
            float3 _350 = Output.xyz + FalloffBlendColor.xyz;
            Output = float4(_350.x, _350.y, _350.z, Output.w);
        }
        else
        {
            if (v_133.fFalloffParam.Param.y == 1.0)
            {
                float3 _363 = Output.xyz - FalloffBlendColor.xyz;
                Output = float4(_363.x, _363.y, _363.z, Output.w);
            }
            else
            {
                if (v_133.fFalloffParam.Param.y == 2.0)
                {
                    float3 _376 = Output.xyz * FalloffBlendColor.xyz;
                    Output = float4(_376.x, _376.y, _376.z, Output.w);
                }
            }
        }
        Output.w *= FalloffBlendColor.w;
    }
    float3 _389 = Output.xyz * v_133.fEmissiveScaling.x;
    Output = float4(_389.x, _389.y, _389.z, Output.w);
    if (Output.w <= Input.AlphaThreshold)
    {
        discard_fragment();
    }
    float3 _417 = mix(v_133.fEdgeColor.xyz * v_133.fEdgeParameter.y, Output.xyz, float3(ceil((Output.w - Input.AlphaThreshold) - v_133.fEdgeParameter.x)));
    Output = float4(_417.x, _417.y, _417.z, Output.w);
    if (Output.w == 0.0)
    {
        discard_fragment();
    }
...
fragment main0_out main0(main0_in in [[stage_in]], constant PS_ConstanBuffer& v_133 [[buffer(0)]], texture2d<float> g_uvDistortionTexture [[texture(0)]], texture2d<float> g_colorTexture [[texture(1)]], texture2d<float> g_normalTexture [[texture(2)]], texture2d<float> g_alphaTexture [[texture(3)]], texture2d<float> g_blendUVDistortionTexture [[texture(4)]], texture2d<float> g_blendTexture [[texture(5)]], texture2d<float> g_blendAlphaTexture [[texture(6)]], sampler g_uvDistortionSampler [[sampler(0)]], sampler g_colorSampler [[sampler(1)]], sampler g_normalSampler [[sampler(2)]], sampler g_alphaSampler [[sampler(3)]], sampler g_blendUVDistortionSampler [[sampler(4)]], sampler g_blendSampler [[sampler(5)]], sampler g_blendAlphaSampler [[sampler(6)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    PS_Input Input;
    ...
    float4 _480 = _main(Input, g_uvDistortionTexture, g_uvDistortionSampler, v_133, g_colorTexture, g_colorSampler, g_normalTexture, g_normalSampler, g_alphaTexture, g_alphaSampler, g_blendUVDistortionTexture, g_blendUVDistortionSampler, g_blendTexture, g_blendSampler, g_blendAlphaTexture, g_blendAlphaSampler);
