...
struct main0_in
{
    ...
    float2 Input_AlphaUV [[attribute(5)]];
    float2 Input_UVDistortionUV [[attribute(6)]];
    float2 Input_BlendUV [[attribute(7)]];
    float2 Input_BlendAlphaUV [[attribute(8)]];
    float2 Input_BlendUVDistortionUV [[attribute(9)]];
    float Input_FlipbookIndex [[attribute(10)]];
    float Input_AlphaThreshold [[attribute(11)]];
};
...
static inline __attribute__((always_inline))
VS_Output _main(VS_Input Input, constant VS_ConstantBuffer& v_280)
{
    ...
    localBinormal = v_280.mCamera * localBinormal;
    localTangent = v_280.mCamera * localTangent;
    float4 cameraPos = v_280.mCamera * pos4;
    ...
    Output.Position = v_280.mProj * cameraPos;
    Output.Pos = Output.Position;
    Output.PosU = v_280.mProj * localBinormal;
    Output.PosR = v_280.mProj * localTangent;
    Output.PosU /= float4(Output.PosU.w);
    Output.PosR /= float4(Output.PosR.w);
    Output.Pos /= float4(Output.Pos.w);
    Output.Color = Input.Color;
    Output.UV = Input.UV;
    Output.UV.y = v_280.mUVInversed.x + (v_280.mUVInversed.y * Input.UV.y);
    Output.AlphaUV = Input.AlphaUV;
    Output.AlphaUV.y = v_280.mUVInversed.x + (v_280.mUVInversed.y * Input.AlphaUV.y);
    Output.UVDistortionUV = Input.UVDistortionUV;
    Output.UVDistortionUV.y = v_280.mUVInversed.x + (v_280.mUVInversed.y * Input.UVDistortionUV.y);
    Output.BlendUV = Input.BlendUV;
    Output.BlendUV.y = v_280.mUVInversed.x + (v_280.mUVInversed.y * Input.BlendUV.y);
    Output.BlendAlphaUV = Input.BlendAlphaUV;
    Output.BlendAlphaUV.y = v_280.mUVInversed.x + (v_280.mUVInversed.y * Input.BlendAlphaUV.y);
    Output.BlendUVDistortionUV = Input.BlendUVDistortionUV;
    Output.BlendUVDistortionUV.y = v_280.mUVInversed.x + (v_280.mUVInversed.y * Input.BlendUVDistortionUV.y);
    float param = Output.FlipbookRate;
    float2 param_1 = Output.FlipbookNextIndexUV;
    float4 param_2 = v_280.mflipbookParameter;
...
vertex main0_out main0(main0_in in [[stage_in]], constant VS_ConstantBuffer& v_280 [[buffer(0)]])
{
    main0_out out = {};
    VS_Input Input;
    ...
    VS_Output flattenTemp = _main(Input, v_280);
