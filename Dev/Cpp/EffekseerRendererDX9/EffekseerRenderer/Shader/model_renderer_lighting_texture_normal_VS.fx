...
static const VS_Output _491 = { 0.0f.xxxx, 0.0f.xx, 0.0f.xxx, 0.0f.xxx, 0.0f.xxx, 0.0f.xxxx, 0.0f.xxxx, 0.0f.xxxx, 0.0f.xxxx, 0.0f.xx };

cbuffer VS_ConstantBuffer : register(b0)
{
    column_major float4x4 _365_mCameraProj : register(c0);
    column_major float4x4 _365_mModel[20] : register(c4);
    float4 _365_fUV[20] : register(c84);
    float4 _365_fAlphaUV[20] : register(c104);
    float4 _365_fUVDistortionUV[20] : register(c124);
    float4 _365_fBlendUV[20] : register(c144);
    float4 _365_fBlendAlphaUV[20] : register(c164);
    float4 _365_fBlendUVDistortionUV[20] : register(c184);
    float4 _365_fFlipbookParameter : register(c204);
    float4 _365_fFlipbookIndexAndNextRate[20] : register(c205);
    float4 _365_fModelAlphaThreshold[20] : register(c225);
    float4 _365_fModelColor[20] : register(c245);
    float4 _365_fLightDirection : register(c265);
    float4 _365_fLightColor : register(c266);
    float4 _365_fLightAmbient : register(c267);
    float4 _365_mUVInversed : register(c268);
};
...
VS_Output _main(VS_Input Input)
{
    ...
    float4 modelColor = _365_fModelColor[Input.Index.x] * Input.Color;
